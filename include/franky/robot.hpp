...
class Robot : public franka::Robot {
 public:
  ...
  [[nodiscard]] std::vector<Vector7d> inverseKinematics(
      ...
      RedundancyParameter parameter = RedundancyParameter::kQ7, const IKOptions &options = {}) {
  ...
  [[nodiscard]] std::optional<Vector7d> inverseKinematicsNearest(
      const Affine &O_T_EE, const IKOptions &options = {},
      std::optional<double> redundancy_value = std::nullopt,
      RedundancyParameter parameter = RedundancyParameter::kQ7,
      std::optional<double> max_distance = std::nullopt) {
    auto s = state();
    return franky::inverseKinematicsNearest(
        O_T_EE, s.q, s.F_T_EE, options, redundancy_value, parameter, max_distance);
  }
